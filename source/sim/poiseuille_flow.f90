...
contains
    ...
    subroutine fuzed_pull_streaming_collision_outer_PF( &
        ...
        ! bottom row
        do x = 1, n_x_local
            call stream_collide_outer_cell_PF(x, 1_int32)
        end do

        ! top row
        if (n_y_local > 1) then
            do x = 1, n_x_local
                call stream_collide_outer_cell_PF(x, n_y_local)
            end do
        end if

        ! left column, excluding corners
        do y = 2, n_y_local - 1
            call stream_collide_outer_cell_PF(1_int32, y)
        end do

        ! right column, excluding corners
        if (n_x_local > 1) then
            do y = 2, n_y_local - 1
                call stream_collide_outer_cell_PF(n_x_local, y)
            end do
        end if
