...
contains
    ...
    subroutine fuzed_pull_streaming_collision_local_CF( &
        n_x_local, n_y_local, at_bottom_boundary, at_top_boundary, &
        ...
        do y = 1, n_y_local
            do x = 1, n_x_local
                ...
                ! (local x-periodic and y-interior handling through halo cells)
                !DIR$ UNROLL(9)
                do i = 1, N_DIRS
                    ...
                    if (src_y >= 1 .and. src_y <= n_y_local) then
    ...
    end subroutine fuzed_pull_streaming_collision_local_CF


    subroutine fuzed_pull_streaming_collision_local_unrolled_CF( &
        n_x_local, n_y_local, at_bottom_boundary, at_top_boundary, &
        write_macro_fields, rho_0, omega, u_wall, f, f_next, rho, u_x, u_y &
        )
        ! inputs
        ...
        do y = 1, n_y_local
            ...
            do x = 1, n_x_local
                ...
                ! ---------
                ! pull streamed distribution functions from source cells in all channels
                ! (local x-periodic and y-interior handling through halo cells, manually unrolled)
                f_1 = f(x, y, 1)
                f_2 = f(x - 1, y, 2)
                f_4 = f(x + 1, y, 4)
    ...
    end subroutine fuzed_pull_streaming_collision_local_unrolled_CF


    subroutine fuzed_pull_streaming_collision_outer_CF( &
        n_x_local, n_y_local, at_bottom_boundary, at_top_boundary, &
        write_macro_fields, rho_0, omega, u_wall, f, f_next, rho, u_x, u_y &
        )
        ! inputs
        ...
        ! bottom row
        do x = 1, n_x_local
            call stream_collide_outer_cell_CF(x, 1_int32)
        end do

        ! top row
        if (n_y_local > 1) then
            do x = 1, n_x_local
                call stream_collide_outer_cell_CF(x, n_y_local)
            end do
        end if

        ! left column, excluding corners
        do y = 2, n_y_local - 1
            call stream_collide_outer_cell_CF(1_int32, y)
        end do

        ! right column, excluding corners
        if (n_x_local > 1) then
            do y = 2, n_y_local - 1
                call stream_collide_outer_cell_CF(n_x_local, y)
            end do
        end if

    contains ! helper subroutine

        subroutine stream_collide_outer_cell_CF( &
            x, y &
            )
            ! inputs
    ...
    end subroutine fuzed_pull_streaming_collision_outer_CF
    ...
    subroutine prepare_couette_flow_halos_CF( &
        ...
        ! bottom wall bounce-back, written into the halo row used by pull streaming
        ...
        ! top wall bounce-back, written into the halo row used by pull streaming
