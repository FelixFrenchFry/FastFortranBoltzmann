...
contains
    ...
    subroutine fuzed_pull_streaming_collision_local_SL( &
        ...
        do y = 1, n_y_local
            do x = 1, n_x_local
                call stream_collide_local_cell_SL(x, y)
            end do
        end do

    contains ! helper subroutine

        subroutine stream_collide_local_cell_SL( &
            x, y &
            )
            ! inputs
            ...
            ! (interior image boundaries are handled through halo cells)
            !DIR$ UNROLL(9)
            do i = 1, N_DIRS
                ...
                if (src_y < 1 .and. at_bottom_boundary) then
                    select case (i)
                    case (3)
                        f_pulled(i) = f(x, y, 5)
                    case (6)
         ...
         end subroutine stream_collide_local_cell_SL
    ...
    subroutine fuzed_pull_streaming_collision_local_unrolled_SL( &
        ...
        has_global_boundary = at_left_boundary .or. at_right_boundary .or. at_bottom_boundary .or. at_top_boundary

        if (.not. has_global_boundary) then
            ! loop over all owned local cells without wall checks
            do y = 1, n_y_local
                do x = 1, n_x_local
                    ...
                    ! ---------
                    ! pull streamed distribution functions from source cells in all channels
                    ! (interior image boundaries are handled through halo cells, manually unrolled)
                    f_1 = f(x, y, 1)
                    f_2 = f(x - 1, y, 2)
                    f_3 = f(x, y - 1, 3)
                    f_4 = f(x + 1, y, 4)
            ...
            do y = 1, n_y_local
                ...
                do x = 1, n_x_local
                    ...
                    ! ---------
                    ! pull streamed distribution functions from source cells in all channels
                    ! (wall boundaries are handled before image-boundary halo cells, manually unrolled)
                    f_1 = f(x, y, 1)
    ...
    subroutine fuzed_pull_streaming_collision_outer_SL( &
        ...
        ! bottom row
        do x = 1, n_x_local
            call stream_collide_outer_cell_SL(x, 1_int32)
        end do

        ! top row
        if (n_y_local > 1) then
            do x = 1, n_x_local
                call stream_collide_outer_cell_SL(x, n_y_local)
            end do
        end if

        ! left column, excluding corners
        do y = 2, n_y_local - 1
            call stream_collide_outer_cell_SL(1_int32, y)
        end do

        ! right column, excluding corners
        if (n_x_local > 1) then
            do y = 2, n_y_local - 1
                call stream_collide_outer_cell_SL(n_x_local, y)
            end do
        end if
    ...
    subroutine prepare_sliding_lid_halos_SL( &
        ...
        ! global left boundary (static), written to pull-streaming halo column
        ...
        ! global right boundary (static), written to pull-streaming halo column
        ...
        ! global top boundary (moving), written to pull-streaming halo row
