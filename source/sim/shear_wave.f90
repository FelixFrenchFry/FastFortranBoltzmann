...
contains
    ...
    subroutine fuzed_pull_streaming_collision_local_SW( &
    ...
    end subroutine fuzed_pull_streaming_collision_local_SW


    subroutine fuzed_pull_streaming_collision_local_unrolled_SW( &
        n_x_local, n_y_local, write_macro_fields, omega, f, f_next, rho, u_x, u_y &
        )
        ! inputs
        integer(int32), intent(in) :: n_x_local
    ...
    end subroutine fuzed_pull_streaming_collision_local_unrolled_SW


    subroutine fuzed_pull_shift_streaming_collision_local_SW( &
        n_x_local, n_y_local, write_macro_fields, omega, f, f_next, rho, u_x, u_y &
        )
        ! inputs
        integer(int32), intent(in) :: n_x_local
        ...
        ! | 8 5 9 |
        ! ---------
        ! pull-style streaming of owned local cells through halo cells into f_next (temporarily)
        do y = 1, n_y_local
            f_next(1:n_x_local, y, 2) = f(0:n_x_local-1, y, 2)
            f_next(1:n_x_local, y, 3) = f(1:n_x_local, y - 1, 3)
            f_next(1:n_x_local, y, 4) = f(2:n_x_local+1, y, 4)
            f_next(1:n_x_local, y, 5) = f(1:n_x_local, y + 1, 5)
            f_next(1:n_x_local, y, 6) = f(0:n_x_local-1, y - 1, 6)
            f_next(1:n_x_local, y, 7) = f(2:n_x_local+1, y - 1, 7)
            f_next(1:n_x_local, y, 8) = f(2:n_x_local+1, y + 1, 8)
            f_next(1:n_x_local, y, 9) = f(0:n_x_local-1, y + 1, 9)
        end do
    ...
    end subroutine fuzed_pull_shift_streaming_collision_local_SW


    subroutine fuzed_pull_shift_streaming_collision_local_unrolled_SW( &
        n_x_local, n_y_local, write_macro_fields, omega, f, f_next, rho, u_x, u_y &
        )
        ! inputs
        integer(int32), intent(in) :: n_x_local
    ...
    end subroutine fuzed_pull_shift_streaming_collision_local_unrolled_SW
    ...
    subroutine fuzed_pull_streaming_collision_outer_SW( &
        ...
        ! bottom row
        do x = 1, n_x_local
            call stream_collide_outer_cell_SW(x, 1_int32)
        end do

        ! top row
        if (n_y_local > 1) then
            do x = 1, n_x_local
                call stream_collide_outer_cell_SW(x, n_y_local)
            end do
        end if

        ! left column, excluding corners
        do y = 2, n_y_local - 1
            call stream_collide_outer_cell_SW(1_int32, y)
        end do

        ! right column, excluding corners
        if (n_x_local > 1) then
            do y = 2, n_y_local - 1
                call stream_collide_outer_cell_SW(n_x_local, y)
            end do
        end if
    ...
    contains ! helper subroutine
        ...
        subroutine stream_collide_outer_cell_SW( &
            ...
            ! ---------
            ! pull streamed distribution functions from source cells in all channels
            ! (periodic boundary handling through halo cells)
            !DIR$ UNROLL(9)
            do i = 1, N_DIRS
