...
contains
    ...
    subroutine execute_full_sim_step( &
        ...
        case (SIM_COUETTE_FLOW)
            if (USE_PUSH_STREAMING) then
                error stop "error: push-streaming couette flow is not implemented yet"
            else if (USE_UNROLLED_KERNELS) then
                error stop "error: unrolled pull-streaming couette flow is not implemented yet"
            else
                call fuzed_pull_streaming_collision_inner_universal( &
        ...
        case (SIM_POISEUILLE_FLOW)
            if (USE_PUSH_STREAMING) then
                error stop "error: push-streaming poiseuille flow is not implemented yet"
            else if (USE_UNROLLED_KERNELS) then
                error stop "error: unrolled pull-streaming poiseuille flow is not implemented yet"
            else
                call fuzed_pull_streaming_collision_inner_universal( &
        ...
        case (SIM_SLIDING_LID)
            if (USE_PUSH_STREAMING) then
                error stop "error: push-streaming sliding lid is not implemented yet"
            else if (USE_UNROLLED_KERNELS) then
                error stop "error: unrolled pull-streaming sliding lid is not implemented yet"
            else
                call fuzed_pull_streaming_collision_inner_universal( &
    ...
    subroutine fuzed_push_streaming_collision_inner_universal( &
        omega, f, f_next &
        )
        ! inputs
        real(FP), intent(in) :: omega
        ...
        do y = 2, N_Y - 1
            do x = 2, N_X - 1
                ...
                f_local = [f_1, f_2, f_3, f_4, f_5, f_6, f_7, f_8, f_9]
                ...
                do i = 1, N_DIRS
                    ...
                    f_next(x + C_X(i), y + C_Y(i), i) = f_next_val
                end do
            end do
        end do
    end subroutine fuzed_push_streaming_collision_inner_universal


    subroutine fuzed_unrolled_push_streaming_collision_inner_universal( &
        omega, f, f_next &
        )
        ! inputs
        real(FP), intent(in) :: omega
        ...
        do y = 2, N_Y - 1
            do x = 2, N_X - 1
                ...
                ! 2: (1, 0)
                f_next(x + 1, y, 2) = f_2 - omega * (f_2 - (1.0_FP/9.0_FP) * rho_val * ( &
                    1.0_FP + 3.0_FP * u_x_val + 4.5_FP * u_x_val * u_x_val - &
                    1.5_FP * u_squ))

                ! 3: (0, 1)
                f_next(x, y + 1, 3) = f_3 - omega * (f_3 - (1.0_FP/9.0_FP) * rho_val * ( &
                    1.0_FP + 3.0_FP * u_y_val + 4.5_FP * u_y_val * u_y_val - &
                    1.5_FP * u_squ))

                ! 4: (-1, 0)
                f_next(x - 1, y, 4) = f_4 - omega * (f_4 - (1.0_FP/9.0_FP) * rho_val * ( &
                    1.0_FP - 3.0_FP * u_x_val + 4.5_FP * u_x_val * u_x_val - &
                    1.5_FP * u_squ))

                ! 5: (0, -1)
                f_next(x, y - 1, 5) = f_5 - omega * (f_5 - (1.0_FP/9.0_FP) * rho_val * ( &
                    1.0_FP - 3.0_FP * u_y_val + 4.5_FP * u_y_val * u_y_val - &
                    1.5_FP * u_squ))

                ! 6: (1, 1)
                f_next(x + 1, y + 1, 6) = f_6 - omega * (f_6 - (1.0_FP/36.0_FP) * rho_val * ( &
                    1.0_FP + 3.0_FP * (u_x_val + u_y_val) + &
                    4.5_FP * (u_x_val + u_y_val) * (u_x_val + u_y_val) - &
                    1.5_FP * u_squ))

                ! 7: (-1, 1)
                f_next(x - 1, y + 1, 7) = f_7 - omega * (f_7 - (1.0_FP/36.0_FP) * rho_val * ( &
                    1.0_FP + 3.0_FP * (-u_x_val + u_y_val) + &
                    4.5_FP * (-u_x_val + u_y_val) * (-u_x_val + u_y_val) - &
                    1.5_FP * u_squ))

                ! 8: (-1, -1)
                f_next(x - 1, y - 1, 8) = f_8 - omega * (f_8 - (1.0_FP/36.0_FP) * rho_val * ( &
                    1.0_FP - 3.0_FP * (u_x_val + u_y_val) + &
                    4.5_FP * (u_x_val + u_y_val) * (u_x_val + u_y_val) - &
                    1.5_FP * u_squ))

                ! 9: (1, -1)
                f_next(x + 1, y - 1, 9) = f_9 - omega * (f_9 - (1.0_FP/36.0_FP) * rho_val * ( &
                    1.0_FP + 3.0_FP * (u_x_val - u_y_val) + &
                    4.5_FP * (u_x_val - u_y_val) * (u_x_val - u_y_val) - &
                    1.5_FP * u_squ))
            end do
        end do
    end subroutine fuzed_unrolled_push_streaming_collision_inner_universal


    subroutine compute_macroscopic_fields( &
        f, rho, u_x, u_y &
        )
        ! inputs
        real(FP), intent(in) :: f(N_X, N_Y, N_DIRS)
        ...
        do y = 1, N_Y
            do x = 1, N_X
                ...
                ! read distribution functions from this cell in all channels
                f_1 = f(x, y, 1)
                f_2 = f(x, y, 2)
                f_3 = f(x, y, 3)
                f_4 = f(x, y, 4)
            ...
            #ifdef FFB_DENSITY_CHECKS
                if (rho_val <= 0.0_FP) then
                    error stop "error: density is zero in macroscopic field computation (rho_val <= 0)"
                end if
            #endif

                ! finalize and store density and velocity
                rho(x, y) = rho_val
                u_x(x, y) = u_x_val / rho_val
                u_y(x, y) = u_y_val / rho_val
            end do
        end do
    end subroutine compute_macroscopic_fields
