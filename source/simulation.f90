module simulation
    ...
    use poiseuille_flow, only: prepare_poiseuille_flow_halos_PF, update_poiseuille_flow_macro_strips_PF, &
        ...
        fuzed_pull_streaming_collision_local_unrolled_PF, fuzed_pull_streaming_collision_outer_PF
    use sliding_lid, only: prepare_sliding_lid_halos_SL, fuzed_pull_streaming_collision_local_SL, &
        fuzed_pull_streaming_collision_local_unrolled_SL, fuzed_pull_streaming_collision_outer_SL
    implicit none
...
contains
    ...
    subroutine fuzed_pull_streaming_collision_inner_universal( &
        ...
        if (n_x_local <= 2 .or. n_y_local <= 2) then
            return
        end if

        ! loop over all owned local inner cells
        do y = 2, n_y_local - 1
            do x = 2, n_x_local - 1
    ...
    subroutine fuzed_pull_streaming_collision_inner_unrolled_universal( &
        n_x_local, n_y_local, write_macro_fields, omega, f, f_next, rho, u_x, u_y &
        )
        ! inputs
        integer(int32), intent(in) :: n_x_local
    ...
    end subroutine fuzed_pull_streaming_collision_inner_unrolled_universal
